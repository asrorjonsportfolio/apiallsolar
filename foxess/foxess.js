...
async function calculateSignature(path, apiKey, timestamp) {
...
module.exports = {
    getDeviceList: async function () {
        let date = Date.now();
        let data = await fetch('https://www.foxesscloud.com/op/v0/device/list', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'token': token,
                'signature': await calculateSignature('/op/v0/device/list', token, date),
                'timestamp': date,
                'lang': 'en',
    ...
    getCurrentData: async function (deviceSn) {
        let date = Date.now();
        let data = await fetch('https://www.foxesscloud.com/op/v0/device/real/query', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'token': token,
                'signature': await calculateSignature('/op/v0/device/real/query', token, date),
                'timestamp': date,
                'lang': 'en',
                'Timezone': 'Asia/Tashkent'
            },
