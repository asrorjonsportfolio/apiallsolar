...
const {
    ...
    getDc,
    getHistoryData,
    getCurrentData, getUserCustomParamData
...
routerH.post('/getDeviceList', (req, res) => {
    getDeviceList()
        .then((response) => {
            ...
            response.result.records.map(async (inverter) => {
                const data = {
                    id: inverter.id,
                ...
                if (find.data.document !== null) {
                    ...
                    callMongo(updateConfig)
                        .then(response => console.log(`${inverter.name} successfully updated`))
                        .catch(error => res.status(401).send({err: error}));
                } else {
                    const Config = config('POST', 'insertOne', insertOne('Cluster0', 'monitoring', `${inverter.powerPlantId}`, data));
                    callMongo(Config)
                        .then(response => console.log(`${inverter.name} successfully added`))
                        .catch(error => res.status(401).send({err: error}));
                }
            });
